...
// Helper: Check if current process has permission
...
int check_permission(struct inode *ip, int mode)
{
  ...
  // Check read permission (needed for O_RDONLY and O_RDWR)
  if(mode == O_RDONLY || mode == O_RDWR){
    if(!owner_read)
      return -1;
  }
  
  // Check write permission (needed for O_WRONLY and O_RDWR)
...
uint64 sys_chmod(void)
{
  ...
  ilock(ip);
  // Update permissions (keep only lower 9 bits)
